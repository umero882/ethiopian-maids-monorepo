/**
 * Admin Financial Payouts Page - Production Version
 * Uses real GraphQL data via adminPayoutsService
 */
...
const log = createLogger('AdminFinancialPayoutsPage');
...
const AdminFinancialPayoutsPage = () => {
  ...
  const loadPayoutsData = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        log.error('Error loading payouts:', error);
        toast({
          title: 'Error',
  ...
  const payoutSummary = useMemo(() => {
    ...
    return {
      totalAmount: stats.total?.amount || 0,
      completedAmount: stats.completed?.amount || 0,
      pendingAmount: stats.pending?.amount || 0,
      totalFees: stats.total?.fees || 0
    };
  }, [stats]);
  ...
  const handlePayoutAction = async (payoutId, action) => {
    ...
    try {
      ...
      log.error('Error updating payout:', error);
      toast({
        title: 'Error',
  ...
  const handleExport = () => {
    try {
      adminPayoutsService.exportToCSV(payoutsData);
      toast({
        title: 'Export Complete',
        description: 'Payouts data has been exported to CSV.',
      });
    } catch (err) {
      toast({
        title: 'Export Failed',
        description: 'Failed to export payouts data.',
        variant: 'destructive',
      });
    }
  };
  ...
  const getPayoutMethodBadge = (method) => {
    ...
    const config = methodConfig[method] || { label: method || 'Unknown', icon: Banknote, color: 'bg-gray-100 text-gray-800' };
    const Icon = config.icon;
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground font-normal">{payout.description}</p>
              </div>
            </DialogTitle>
          </DialogHeader>
    ...
    <div className="space-y-6">
      ...
      <Alert className="border-green-200 bg-green-50/50">
        ...
        <AlertDescription className="text-green-700">
          <strong>Live Data:</strong> Payouts are loaded from the database.
          {payoutsData.length === 0 && ' No payouts found in the system.'}
        </AlertDescription>
      </Alert>
                    ...
                    <TableRow key={payout.id}>
                      ...
                      <TableCell>
                        <div className="text-sm">
                          <div className="font-medium">{payout.payout_number || payout.id?.substring(0, 8)}</div>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {payout.status === 'pending' && (
                              <>
                                <DropdownMenuItem onClick={() => handlePayoutAction(payout.id, 'approve')}>
                                  <CheckCircle2 className="mr-2 h-4 w-4 text-green-500" />
                                  Approve
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handlePayoutAction(payout.id, 'hold')}>
                                  <Pause className="mr-2 h-4 w-4 text-orange-500" />
                                  Hold
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handlePayoutAction(payout.id, 'reject')}>
                            ...
                            {payout.status === 'failed' && (
                              <DropdownMenuItem onClick={() => handlePayoutAction(payout.id, 'retry')}>
                                <ArrowUpRight className="mr-2 h-4 w-4 text-blue-500" />
                                Retry
                              </DropdownMenuItem>
                            )}
                            {payout.status === 'on_hold' && (
                              <DropdownMenuItem onClick={() => handlePayoutAction(payout.id, 'release')}>
