...
**Deploying to production (CRITICAL - stale cache prevention):**

```bash
# 1. ALWAYS clean Nx output dir and reset cache BEFORE building
rm -rf dist/apps/web
pnpm nx reset
...
# 3. On server, clean old assets BEFORE extracting new build
sudo rm -rf /var/www/ethiopianmaids/assets
sudo rm -f /var/www/ethiopianmaids/sw.js /var/www/ethiopianmaids/workbox-*.js

# 4. Extract new build
sudo tar xzf /tmp/web-deploy.tar.gz -C /var/www/ethiopianmaids/
```

> **Why?** The Nx output dir (`dist/apps/web/`) accumulates old build artifacts across builds.
> The Vite PWA plugin scans this dir to generate `sw.js` precache manifest. Without cleaning,
> `sw.js` bloats (452KB/7764 files vs 14KB/206 files) and serves stale cached JS to users.
