...
export interface MaidProfileProps {
  id: string;
  userId: string;
...
export class MaidProfile {
  ...
  constructor(props: MaidProfileProps) {
    this.id = props.id;
    this.userId = props.userId;
    this.fullName = props.fullName || null;
    this.dateOfBirth = props.dateOfBirth ? new Date(props.dateOfBirth) : null;
   ...
   */
  updateBasicInfo({ fullName, dateOfBirth, nationality, phone }: BasicInfoUpdate): void {
   ...
   * Upload document
   */
  uploadDocument(type: 'passport' | 'medicalCertificate' | 'policeClearance', documentUrl: string): void {
