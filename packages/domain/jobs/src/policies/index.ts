...
import type { Salary } from '../value-objects/Salary.js';
import type { JobPosting, MaidProfileForMatching } from '../entities/JobPosting.js';

/**
 * Minimum salary configuration
 */
...
/**
 * Maid profile for application validation
 */
export interface MaidProfileForApplication {
...
export const JobPostingPolicies = {
  ...
  } as Record<string, MinimumSalary>,
   ...
   */
  validateSalary(salary: Salary, country: string): SalaryValidationResult {
   ...
   */
  isJobComplete(job: JobPosting): boolean {
    return !!(
      job.title &&
      job.description &&
   ...
   */
  validateExpiryDays(days: number): boolean {
   ...
   */
  shouldAutoExpire(job: JobPosting): boolean {
   ...
   */
  getRecommendedSalaryRange(experienceYears: number, country: string): SalaryRange | null {
    const countryCode = country.substring(0, 2).toUpperCase();
    const minimum = this.MINIMUM_SALARY[countryCode];
...
export const ApplicationPolicies = {
   ...
   */
  canMaidApplyToJob(maidProfile: MaidProfileForApplication, job: JobPosting): ApplicationEligibilityResult {
    const errors: string[] = [];
  ...
  calculatePriorityScore(
    application: { matchScore: number; appliedAt: Date },
    maidProfile: MaidProfileForMatching & { isVerified: boolean }
  ): number {
    let score = 0;
   ...
   */
  isMatchScoreAcceptable(score: number): boolean {
...
export const MatchingPolicies = {
   ...
   */
  getRecommendationThreshold(): number {
   ...
   */
  getAutoSuggestionThreshold(): number {
