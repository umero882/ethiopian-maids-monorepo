...
const AgencyApplicantsPage = () => {
    ...
    const fetchApplicantsAndJobs = async () => {
      try {
        ...
        const transformedApplications = (applicationsResult.data || []).map(app => ({
          ...
          age: calculateAge(app.maid?.date_of_birth) || 0,
  ...
  const filteredApplicants = applicants.filter((applicant) => {
    ...
    if (searchTerm && searchTerm.trim() !== '') {
      ...
      const matchesSearch =
        (applicant.maid_name && applicant.maid_name.toLowerCase().includes(term)) ||
        (applicant.job_title && applicant.job_title.toLowerCase().includes(term)) ||
        (applicant.nationality && applicant.nationality.toLowerCase().includes(term)) ||
        (applicant.skills && applicant.skills.some((skill) => skill.toLowerCase().includes(term)));
      if (!matchesSearch) return false;
    }

    // Status filter
    if (statusFilter && statusFilter !== 'all' && applicant.application_status !== statusFilter) {
      return false;
    }

    // Job filter
    if (jobFilter && jobFilter !== 'all' && applicant.job_id.toString() !== jobFilter) {
      return false;
    }
