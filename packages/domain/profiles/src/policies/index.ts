...
import type { WorkExperience } from '../value-objects/WorkExperience.js';
import type { ProfileStatusType } from '../value-objects/ProfileStatus.js';
...
export const ProfilePolicies = {
   ...
   */
  isValidMaidAge(dateOfBirth: Date | string | null): boolean {
   ...
   */
  calculateAge(dateOfBirth: Date | string | null): number | null {
   ...
   */
  areSkillsValid(skills: string[]): boolean {
   ...
   */
  areLanguagesValid(languages: string[]): boolean {
   ...
   */
  hasMinimumWorkExperience(workExperienceArray: WorkExperience[]): boolean {
   ...
   */
  isValidHouseholdSize(size: number | null): boolean {
   ...
   */
  isValidCountryCode(code: string | null): boolean {
   ...
   */
  canSubmitProfile(completionPercentage: number): boolean {
...
export const StatusTransitionPolicies = {
   ...
   */
  isTransitionAllowed(currentStatus: ProfileStatusType, newStatus: ProfileStatusType): boolean {
    const allowedTransitions: Record<ProfileStatusType, ProfileStatusType[]> = {
      draft: ['under_review'],
   ...
   */
  getAllowedNextStatuses(currentStatus: ProfileStatusType): ProfileStatusType[] {
    const allowedTransitions: Record<ProfileStatusType, ProfileStatusType[]> = {
      draft: ['under_review'],
      under_review: ['active', 'rejected'],
      active: ['archived'],
